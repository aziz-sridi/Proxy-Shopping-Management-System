...
public class OrderDialogs {
    ...
    /**
     * Open dialog to edit an existing order.
     */
    ...
    private void openOrderFormDialog(Order editingOrder,
        ...
        if (isEditMode) txtProduct.setText(editingOrder.getProductLink());

        TextField txtSize = DialogUtils.createStyledTextField("Size");
        if (isEditMode) txtSize.setText(editingOrder.getProductSize());

        Spinner<Integer> spQty = new Spinner<>(1, 1000, isEditMode ? editingOrder.getQuantity() : 1);
        spQty.getStyleClass().add("app-field");

        TextField txtOriginal = DialogUtils.createStyledTextField("Original price (EUR)");
        if (isEditMode) txtOriginal.setText(String.valueOf(editingOrder.getOriginalPrice()));

        TextField txtSelling = DialogUtils.createReadOnlyTextField("Selling price (TND)", 
            "-fx-background-color: #f0f0f0; -fx-font-weight: bold;");
        if (isEditMode) txtSelling.setText(String.valueOf(editingOrder.getSellingPrice()));

        ComboBox<String> cbPaymentType = new ComboBox<>();
        cbPaymentType.getStyleClass().add("app-field");
        cbPaymentType.getItems().addAll("Deposit", "Full", "On Delivery");
        if (isEditMode) {
            cbPaymentType.setValue(editingOrder.getPaymentType());
        } else {
            cbPaymentType.setValue("On Delivery");
        }

        ComboBox<String> cbPlatform = new ComboBox<>();
        cbPlatform.getStyleClass().add("app-field");
        cbPlatform.getItems().addAll(Platform.getDisplayNames());
        if (isEditMode) {
            cbPlatform.setValue(editingOrder.getPlatform() != null ? editingOrder.getPlatform().getDisplayName() : "Other");
        } else {
        ...
        if (isEditMode) txtNotes.setText(editingOrder.getNotes());
        ...
        btnCalculate.getStyleClass().addAll("app-button", "button-secondary");
        ...
        dialog.setResultConverter(btn -> {
            if (btn == ButtonType.OK) {
                if (isEditMode) {
                    saveEditOrder(editingOrder, cbClient.getValue(), cbShipment.getValue(), cbDelivery.getValue(),
                        txtProduct.getText(), txtSize.getText(), spQty.getValue(),
                        txtOriginal.getText(), txtSelling.getText(), cbPaymentType.getValue(),
                        cbPlatform.getValue(), txtNotes.getText(), onSuccess);
                } else {
    ...
    private void saveEditOrder(Order order, Client client, Shipment shipment, DeliveryOption delivery,
     ...
     * Open dialog to create a new order.
     */
    public void openNewOrderDialogLegacy(ObservableList<Client> clients, 
                                    ObservableList<Shipment> shipments,
                                    ObservableList<DeliveryOption> deliveryOptions,
                                    OrderCallback onSuccess,
                                    OrderCallback onPaymentCreated) {
